...
const slogan = appSettings?.slogan || '';
const receiptHeader = appSettings?.receiptHeader || '';
const receiptFooter = appSettings?.receiptFooter || 'Merci de votre visite !';
...
${slogan ? `<div style="font-size:10px; color:#666; margin-bottom:4px;">${slogan}</div>` : ''}
<div style="margin-top:6px;">Ticket de caisse</div>
</div>
...
${payment.method !== 'CASH' && payment.reference ? `<div style="font-size:10px; color:#666; margin-left:8px;">Réf: ${payment.reference}</div>` : ''}
`).join('')}
</div>
` : ''}

<div class="footer">
${receiptHeader ? `<div style="margin-bottom:4px; font-style:italic;">${receiptHeader}</div>` : ''}
<div>${receiptFooter}</div>
${companyPhone ? `<div style="margin-top: 6px;">Tél: ${companyPhone}</div>` : ''}
